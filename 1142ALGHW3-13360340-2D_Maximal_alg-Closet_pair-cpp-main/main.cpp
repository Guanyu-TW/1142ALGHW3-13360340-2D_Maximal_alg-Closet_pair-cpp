...
vector<Point> maximalPoints(const vector<Point>& S) {
    ...
    vector<Point> SL, SR;
    for (const auto& p : S) {
        // 將 x <= medianX 的點放入 SL，x > medianX 的點放入 SR
        if (p.x <= medianX) {
            SL.push_back(p);
        } else {
            SR.push_back(p);
    ...
    if (rightMaximal.empty()) {
        ...
        for (const auto& p : leftMaximal) {
            // 注意：由於 SR 的 x 嚴格大於 SL，若 y 相同也會被凌駕，因此嚴格條件為 p.y > ymax
            if (p.y > ymax) {
    ...
    for (const auto& p : filteredLeft) result.push_back(p);
    for (const auto& p : rightMaximal) result.push_back(p);
...
double closestPairRecursive(vector<Point> Px, vector<Point> Py) {
    ...
    // Py 已按 y 排序，所以此處建立出來的 strip 自動擁有依 y 排序的特性
    for (const auto& p : Py) {
        if (abs(p.x - L) < delta) {
            strip.push_back(p);
        }
    ...
    int m = strip.size();
    for (int i = 0; i < m; i++) {
        // 對 strip[i] 後面有限個點進行比較，條件確保了只比較 y 軸差距小於 delta 的點
